...
import { findShortestPathBetween, main, parseInput } from "./2.code";
...
it("works for example input", () => {
  expect(main(sampleInput, 10)).toBe(1030);
});
it("works for example input 2", () => {
  expect(main(sampleInput, 100)).toBe(8410);
});

it("works for real input", async () => {
  const input = await readFile(join(__dirname, "input.txt"), "utf-8");
  expect(main(input)).toBeGreaterThan(9108609);
  expect(main(input)).toBeGreaterThan(9724940);
  expect(main(input)).toBe(569052586852);
});
