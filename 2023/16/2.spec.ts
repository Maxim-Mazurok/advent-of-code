...
import { main, parseInput, solve, visualize } from "./2.code";
...
it("shows energized grid", () => {
  const result = solve(parseInput(sampleInput), 0, 0, "right");
...
it("works for example input", () => {
  expect(main(sampleInput)).toBe(51);
});

it("works for real input", async () => {
  const input = await readFile(join(__dirname, "input.txt"), "utf-8");
  expect(main(input)).toBe(7853);
});
